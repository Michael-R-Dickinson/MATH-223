...
By the construction of the Gram-Schmidt procedure, each $v_i$ is formed by taking $u_i$ and subtracting off projections onto the previous vectors, then normalizing. This means $v_i$ is a linear combination of $u_1, \ldots, u_i$. In particular, for $i \leq k$, the vector $v_i$ is a linear combination of $u_1, \ldots, u_i$, all of which are in $U$. Since $U$ is a subspace, $v_i \in U$ for all $i \leq k$.

Thus $(v_1, \ldots, v_k)$ is a list of $k$ orthonormal vectors contained in $U$. Since $\dim U = k$ (because $(u_1, \ldots, u_k)$ is a basis), and any $k$ orthonormal vectors in a $k$-dimensional space must be a basis, we conclude that $(v_1, \ldots, v_k)$ is an orthonormal basis of $U$.
