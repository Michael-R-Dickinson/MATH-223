...
We prove the parallelogram law by expanding both sides using the inner product.

Starting with the left side, we compute $\norm{v+w}^2$:
\[
...
Since we are working over the reals, $\langle v\mid w \rangle = \langle w\mid v \rangle$, so:
\[
\norm{v+w}^2 = \norm{v}^2 + 2\langle v\mid w \rangle + \norm{w}^2
\]
...
The inner product terms cancel:
\[
= 2\norm{v}^2 + 2\norm{w}^2 = 2(\norm{v}^2 + \norm{w}^2)
\]
